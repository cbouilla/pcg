\documentclass[nohyperref,preprint,svgnames]{iacrtrans}
...
\begin{align}
  ...
  \label{eq:lcg-scalar}
  \LCG_{n,k}(\lamba x, \lambda c) = \lambda \LCG_{n,k}(x, c). 
...
\textsf{PCG64} generator with known increment proceeds as follows:
  ...
  \begin{algorithmic}[1]
    ...
    \State $ S_0[0:64] \gets  S'_0 + 2^{\ell} \cdot U_0[0:64-\ell]$ \Comment{Reconstruct $S_0$}
    \State $ S_0[64:128] \gets S_0[0:64] \oplus Y_0$ 
    
    \State $ S_1 \gets a  S_0 + c$ \Comment{Check consistency of $X_1$}
...
Set $\Delta S_i = S{i+1} - S_i$; it is easily checked that $\Delta S_i$ is a
...
unknown increment $c$). The prediction algorithm proceeds in three phases:
\begin{enumerate}
\item Reconstruct $\Delta S_0[0:64+\ell]$ from $X_0, \dots, X_4$.
\item Fully reconstruct $\Delta S_0$ from this partial knowledge and $X_0, \dots, X_{39}$.
\item Reconstruct $S_0$ from $\Delta S_0$ and $X_0, \dots, X_{32}$.
\end{enumerate}
...
least significant bits of $c$ as well. We must also guess one additional
rotation to compute the differences $\Delta S_i$.
...
Given a ``fresh'' output $X_i$, we can enumerate the 64 possible rotations
$r_i$; for each one, we can check whether
$(X_i \lll r_i)[0:\ell] = S_i[0:\ell] \oplus S_i[64:64+\ell]$. If none of the
rotations yields a match, then the guesses made beforehand have to be wrong. As
soon as $\ell \geq 6$, this test is efficient (and it can be done for any number
of fresh $X_i$'s). As a consequence, bad guesses can be filtered deterministically.

\begin{align*}
  \mathbf{S} &= \LCG(S_0, c) \\
             &= \LCG\left(S_0[0:\ell] + 2^\ell S_0[\ell:128], c[0:\ell] + 2^\ell c[\ell:128]\right) \\
             &= \LCG\left(S_0[0:\ell], c[0:\ell]) + 2^\ell \LCG\left(S_0[\ell:128],  c[\ell:128]\right) \\
  \Delta \mathbf{S} &= \LCG(S_1, c) - \LCG(S_0, c) \\
             &= \LCG(S_1 - S_0, 0)
\end{align*}

\begin{algorithmic}[1]
  \Procedure{ReconstructPartialDifference}{$X_0, \dots, X_9$}
  ...
  \State $T'_j \gets T_j -  S'_i[58:64+\ell]$ \Comment{Shifted truncated LCG}
  \State $\Delta T'_j = T'_{j+1} - T'_j$  \Comment{Difference}
  \State $(\Delta U_0, \Delta U_1, \Delta U_2, \Delta U_3) \gets \left\lfloor (\Delta T'_0, \Delta T'_1, \Delta T'_2, \Delta T'_3) \cdot 2^{39} \cdot \widetilde G_4^{-1} \right\rceil \cdot \widetilde G_4$ \Comment{CVP}
  \State $\Delta S_0[0:64+\ell] \gets \left((w_1 - w_0) \bmod 2^{\ell}\right) + 2^{\ell} \cdot U_0[0:58-\ell]$
  \State ...
  \EndFor
  \EndFor
  \EndProcedure
