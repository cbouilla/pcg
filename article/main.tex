...
The LCG have been largely studied. This is why we know techniques to recover internal states knowing only upper or lower bits of the output, see \cite{Boyar1989}. A particular LCG is the Lehmer's generator \cite{Lehmer} \todo{je sais pas écire la référence dans bibtex}, where the multiplier \(a\) is known and the increment is zero.
...
\begin{algorithmic}[1]
  ...
  \State $x_0 = w_0 \oplus Y_0[0:\ell]$ \Comment{$x_0 = S_0[64:64+\ell]$}
  \For{$i=5, \dots, k$}
  \State $x_i = x_0 + \left(\Delta S_0 \frac{a^i-1}{a-1} \bmod 2^{64+\ell}\right)[64:64+\ell]$. \Comment{$x_i = S_i[64:64+\ell]$}
%  \State $\Delta S_i[\ell:64+\ell] \gets a^i \Delta U_0 \bmod 2^{64}$
  \State $\mathrm{Target} \gets S_i[0:\ell] \oplus x_i$
  \If{$\mathrm{Target} \not\in \{ (X_i \lll r_i)[0:\ell]~|~ 0\leq r_i < 64\}$}
  % \State $\mathrm{Found} \gets \mathbf{False}$
  % \For{$0 \leq r_i < 64$}
  % \State $Y_i \gets X_i \lll r_i$
  % \If{$Y_i[0:\ell] = \mathrm{Target}$}
  % \State $\mathrm{Found} \gets \mathbf{True}$ 
  % \EndIf
  % \EndFor
  % \If{not Found}
  ...
  \EndIf
  \EndFor
  \State \Return \textbf{True} \Comment{Good  Guesses}
  \EndProcedure
