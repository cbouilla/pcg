...
\aalgbegin Algorithm K (State reconstruction with known increment). Reconstruct $S_0$ from three consecutive outputs $X_j$ ($0 \leq j < 3$). In all the algorithm, we have $j= 0, 1, 2$.

\algstep K1. [Guess state] For each $0 \leq w < 2^{19}$, set $S_0[0 : \ell] = w$ and do steps K2--K10.

\algstep K2. [Propagate state] Set $S_j[0:\ell] \gets a^j w \bmod 2^{19}$.

\algstep K3. [Guess rotations] For each $0 \leq r_0, r_1, r_2 < 64$, do steps K4--K10.

\algstep K4. [Undo rotations] Set $S_j[122:128] \gets r_j$ and $Y_j \gets X_j \lll r_j$.

\algstep K5. [Propagate] Set $S_j[58:64] \gets r_j \oplus Y_j[58:64]$ and $S_j[64:83] \gets S_j[0:19] \oplus Y_j[0:19]$.

\algstep K6. [Build partial $T$] Set $T_j \gets \left(S_j[58:83] - (a^j w + c \sum_{k = 0}^j a^k)[58:83]\right) \cdot 2^{39}$

\algstep K7. [Reconstruct $T$] Set $(U_0, U_1, U_2) \gets \left\lfloor (T_0, T_1, T_2) \cdot \widetilde G^{-1} \right\rceil \cdot \widetilde G$.

\algstep K8. [Reconstruct $S_0$] Set $S_0[19:83] \gets U_0$ and $S_0[83:122] \gets U_0[0:39] \oplus Y_0[19:58]$.

\algstep K9. [Recompute $X_1$] Compute $\widehat S_1 \gets a S_0 + c$ and $\widehat X_1 =(\widehat S_1[0:64] \oplus \widehat S_1[64:128]) \ggg r_1$.

\algstep K10. [Check] If $\widehat{X}_1 = X_1$, the output $S_0$ as a candidate internal state.
\end{minipage}
}
\end{center}
