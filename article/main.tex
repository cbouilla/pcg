...
\subsection{A first way to solve the CVP : the Babai's rounding}
...
By definition, \(r\) is the closest integer vector to \(TG^{-1}\). But as \(U\) is an element of the lattice, \(UG^{-1}\) is an integer vector. Thus \(r-TG^{-1}\) is smaller than \(UG^{-1}-TG^{-1}\).
...
If we can prove that the left side of the inequality is smaller than the SVP of G, then wa have proved that \(rG\) is indeed the vector we were searching for.
