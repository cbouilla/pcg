...
\begin{align*}
|\CVP(\mathcal{L}, 2^{k-\ell}\widetilde{T}) - U'| %&= \left| \CVP\left( \mathcal{L}, 2^{k-\ell}\widetilde{T} \right) \riht| + \left|2^{k-\ell}\widetilde{T} - U'\right| %-2^{k-\ell}\widetilde{T}+2^{k-\ell}\widetilde{T}-U'|\\
...
\State $K_j \gets a^j w_0 + (2c_0 + 1)(a^j - 1)(a-1)^{-1} \bmod 2^{128}$ \Comment{Known part}
  ...
  \State $\widetilde{\Delta T'}_j = \widetilde{T'}_{j+1} \boxminus \widetilde{T}'_j$  \Comment{Difference (truncated geom. seq.)}
  \State $(\Delta U_0, \dots, \Delta U_3) \gets \left\lfloor (\Delta T'_0, \dots, \Delta T'_3) \cdot 2^{58-\ell} \cdot \widetilde H^{-1} \right\rceil \cdot \widetilde H$ \Comment{CVP}
