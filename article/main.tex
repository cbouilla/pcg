...
\begin{algorithmic}[1]
    ...
    \State $S'_j \gets a^j w + c (a^j - 1)(a-1)^{-1} \bmod 2^{128}$    \Comment{Shifted states}
    ...
    \State $(U_0, U_1, U_2) \gets \left\lfloor (T'_0, T'_1, T'_2) \cdot 2^{58-\ell} \cdot \widetilde G_3^{-1} \right\rceil \cdot \widetilde G_3$ \Comment{CVP (Babai rounding)}
...
\Procedure{ReconstructPartialDifference}{$X_0, \dots, X_k$}
  ...
  \State $S'_j \gets a^j w_0 + (2c+1)(a^j - 1)(a-1)^{-1} \bmod 2^{128}$ \Comment{Shifted states}
