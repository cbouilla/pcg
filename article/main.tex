...
\email{julia.sauvage@etu.sorbonne-universite.fr}
}
...
\begin{proof}
  ...
  \item Otherwise, there are two possible ``wraparound'' cases:
    ...
    \item Either $T_i = 0$ and $N_i = -1$, which leads to $\widetilde{T}_i = 2^t-1$. 
      ...
      \[
        \left| 2^{k-t}\widetilde{T}_i-(U_i + 2^k) \right| = %\left| 2^{k-t}(2^t -1) -(U_i + 2^k) \right| =
    ...
    \item Or $T_i = 2^{t}-1$ and $N_i = +1$, which leads to
      ...
      \[
        \left| 2^{k-t}\widetilde{T}_i-(U_i - 2^k) \right| %=\left| 0 -(U_i - 2^k) \right|
...
\begin{align*}
|\CVP(\mathcal{L}, 2^{k-t}\widetilde{T}) - U'| %&= \left| \CVP\left( \mathcal{L}, 2^{k-\ell}\widetilde{T} \right) \riht| + \left|2^{k-\ell}\widetilde{T} - U'\right| %-2^{k-\ell}\widetilde{T}+2^{k-\ell}\widetilde{T}-U'|\\
...
%\noindent But enough fooling around; let's go back to \textsf{PCG64}.
...
less than 26 cycles per iteration of the inner loop. We used several
...
than 54 cycles per iteration of the inner loop. We used essentially the same
...
\paragraph{Acknowledgements.} The authors thank Thomas Espitau, Virgile Tellier,
Damien Vergnaud and Vincent Zucca for helpful discussions. We also thank Léo
Ducas for helping us with the \texttt{G6K} library and Melissa O'Neil for kindly
indulging our request for challenge inputs. We finally thank Alexandra Assanovna
Elbakyan for helping scientists to access the works of their colleagues.
