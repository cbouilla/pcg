...
\todo{NOTE: J'introduis $\ell$ ici. ATTENTION RETENUES MODULAIRES.}  We want to
find an element \(U\) in the lattice only knowing the $\ell$ most significant
bits of each of its coordinates.  More precisely, let $T_i = U_i[k-\ell:k]$
denote the top $\ell$ bits of $U_i$. We assume that there is a ``noise vector''
$N$ such that $N_i \in \{-1, 0, 1\}$, and we set
$\widetilde{T}_i = T_i + N_i \bmod 2^{\ell}$. The problem we have is the
following: recover $U$ from its noisy truncation $\widetilde{T}$.

In principle, $2^{k-\ell} \widetilde{T}$ should be ``close'' to a Lattice
point. \todo{Mais il faut se dire que $2^{k-\ell} T$ et
  $2^{k-\ell} \widetilde{T}$ peuvent être éloignés, surtout si $T_i = 0$ et
  $N_i=-1$, à cause du $\bmod 2^\ell$... L'argument de Julia était que dans ce
  cas-là, $2^{k-\ell} \widetilde{T}$ est proche d'un \emph{autre} point du
  réseau, en ajoutant un $2^k$ sur la $i$-ème composante.}

Decompose $U = T + (T-U)$ where $T$ contains only the upper bits and $T-U$ the lowers one. There is no reason for \(T\) to be in the lattice, but as \(T-U\) is small, we know \(T\) is \emph{close} to \(U\).
...
\todo{En fait, on va juste utiliser $k=128$, donc établir $n=42$ est suffisant. Faire disparaître $\ell$.}
...
eigenvalue of $G^{t} G$ --- \todo{recheck this}).
...
\todo{Ce serait beaucoup plus naturel à mon avis de dire qu'on connaît les $\ell$ bits de poids fort. A ce stade, rien ne vient justifier le ``+6''.}
...
%    \State $w_j = a^j w \bmod 2^{\ell}$ \Comment{Propagate to $S_1, S_2$}
    ...
    \State $\widetilde T'_j \gets T_j \boxminus K_j[58:64+\ell]$  \Comment{Truncated geometric series}
